...
class DigestAuthentication(Authentication):
    ...
    def __init__(self, credentials, host, request_uri, headers, response, content, http):
        ...
        self.A1 = "".join([self.credentials[0], ":", "ARINC", ":", self.credentials[1]])
        self.challenge['nc'] = 1
    ...
    def request(self, method, request_uri, headers, content, cnonce = None):
        ...
        headers['Authorization'] = 'Digest username="%s", realm="ARINC", nonce="%s", uri="%s", algorithm=%s, response=%s, qop=%s, nc=%08x, cnonce="%s"' % (
                self.credentials[0],
...
class HmacDigestAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content):
        ...
        headers['Authorization'] = 'HMACDigest username="%s", realm="ARINC", snonce="%s", cnonce="%s", uri="%s", created="%s", response="%s", headers="%s"' % (
                self.credentials[0],
...
class GAE_Connection:
    ...
    def request_delete(self, resource, args = None, body = None, filename=None, headers={}):
        return self.request(resource, urlfetch.DELETE, args , body = body, filename=filename, headers=headers)
    ...
    def _conn_request(self, conn, request_uri, method, body, headers):
        ...
        urlfetch_response = urlfetch.fetch(request_uri, method=method, payload=body, headers=headers, deadline=60)
